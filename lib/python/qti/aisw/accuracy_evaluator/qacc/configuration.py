...
class QnnInferenceSchemaConfiguration:
    ...
    def validate(self):
        ...
        android_backends = [
            qcc.BACKEND_CPU, qcc.BACKEND_DSPV68, qcc.BACKEND_DSPV69, qcc.BACKEND_DSPV73,
            qcc.BACKEND_DSPV75, qcc.BACKEND_DSPV79, qcc.BACKEND_GPU
        ]
        ...
        if self._target_arch == 'aarch64-android':
            # check if adb binary path from env variable exists or adb from executable path exists
            adb_path = os.environ.get("ADB_PATH", shutil.which('adb'))
            if not os.path.exists(adb_path):
                raise ce.ConfigurationException("Environment variable ADB_PATH not set. \
                                                ADB_PATH is required while evaluating when target_arch is aarch64-android. \
