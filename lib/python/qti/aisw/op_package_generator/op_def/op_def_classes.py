...
class ActivationTensorElement(TensorElement):
    ...
    '''
    def __init__(self, name, description, mandatory, default, datatype, rank, shape, layout=Layout.UNDEFINED, constraints=[], repeated=False, dynamic_shape=""):
    ...
    dynamic_shape = property_type("dynamicshape", str)
...
class InputElement(ActivationTensorElement):
    ...
    def __init__(self, name, description, mandatory, default, datatype, rank,
                 shape, layout=Layout.UNDEFINED, constraints=[], repeated=False, dynamic_shape="", is_static_tensor=False):
        super().__init__(name=name, description=description, mandatory=mandatory, default=default, datatype=datatype, rank=rank, shape=shape, layout=layout, constraints=constraints, repeated=repeated, dynamic_shape=dynamic_shape)
...
class OutputElement(ActivationTensorElement):
    ...
    def __init__(self, name, description, mandatory, datatype, rank, shape, layout=Layout.UNDEFINED, constraints=[], repeated=False, dynamic_shape=""):
        super().__init__(name=name, description=description, mandatory=mandatory, default="", datatype=datatype, rank=rank, shape=shape, layout=layout, constraints=constraints, repeated=repeated, dynamic_shape=dynamic_shape)
...
class OpDefCollection:
    ...
    def get_op_def(self, op_def_name: str, backend: str="ALL", runtime = None) -> OpDef:
        '''
        ...
        if not self.is_supported(op_def_name, backend, runtime):
            raise KeyError("OpDef {} is unsupported for Backend {} and runtime {}.".format(op_def_name, backend, runtime))

        op_def = None
        if backend != self.ALL and self.is_supplemented(op_def_name, backend, runtime):
            supp_op_def = self.get_supplemental_op_def(op_def_name, backend, runtime)
    ...
    def get_backend_op_defs(self, backend: str, runtime = None) -> Dict[str, OpDef]:
        '''
        ...
        '''
        supported = self.get_supported_ops(backend, runtime)
        op_defs = {}
        for op_def_name in supported:
            op_defs[op_def_name] = self.get_op_def(op_def_name, backend, runtime)
        return op_defs
    ...
    def get_supplemental_op_defs(self, backend: str=None, runtime=None) -> Dict[str, SupplementalOpDef]:
        ...
        if runtime is None:
            ...
            if runtime in self.get_supported_runtimes(backend) + [self.ALL]:
