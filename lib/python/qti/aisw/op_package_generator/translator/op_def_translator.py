...
class OpDefTranslator:
    ...
    def __get__activation_tensor_attr(self, elem: ET.Element, \
        ...
        dynamic_shape_elem = elem.find(XmlConstants.DYNAMIC_SHAPE)
        if dynamic_shape_elem is not None and dynamic_shape_elem.find(XmlConstants.DYNAMIC_SHAPE_TEXT) is not None:
            dynamic_shape = self.__flatten_paragraph(dynamic_shape_elem.find(XmlConstants.DYNAMIC_SHAPE_TEXT).text)

        tensor_attrs[XmlConstants.REPEATED] = repeated
        tensor_attrs[XmlConstants.DYNAMIC_SHAPE] = dynamic_shape
        return tensor_attrs
