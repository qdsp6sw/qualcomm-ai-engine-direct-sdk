...
#   BatchToSpace
# ------------------------------------------------------------------------------
class RelayBatchToSpaceTranslation(RelayTranslationBase):
    def __init__(self):
        super(RelayBatchToSpaceTranslation, self).__init__()
    ...
    def extract_attributes(self,
                           ...
                           **kwargs):
        attr_dict = {}
        attr_dict["block_shape"] = relay_expr.attrs.block_shape
        attr_dict["crops"] = relay_expr.attrs.crops

        log_debug3("\tblock_shape {}, crops {}",
                   attr_dict["block_shape"], attr_dict["crops"])
    ...
    def translate_op(self,
        ...
        op_name = converter_context.get_op_name(relay_expr, BatchToSpaceOp.TRANSLATION_KEY,
                                                BatchToSpaceOp.LEGACY_TRANSLATION_KEY)

        block_shape = attr_dict["block_shape"]
        if isinstance(block_shape, tvm.runtime.ndarray.NDArray) or isinstance(block_shape, tvm.runtime.NDArray) \
                or isinstance(block_shape, tvm.ir.container.Array):
            block_shape = np.array(block_shape).astype('int')
            block_shape = [shape.item() for shape in block_shape]

        crops = attr_dict["crops"][:]
        for index, crop in enumerate(crops):
            if isinstance(crop, tvm.ir.container.Array):
                crop = np.array(crop).astype('int')
                crop = [shape.item() for shape in crop]
            crops[index] = crop

        ir_op = BatchToSpaceOp(op_name, block_shape=block_shape, crops=crops)
        return ir_op


RelayTranslations.register_translation(RelayBatchToSpaceTranslation(),
                                       converter_type('batch_to_space_nd', 'relay'))
...
class RelayBiasaddTranslation(RelayTranslationBase):
    ...
    def translate_op(self,
        ...
        op_name = converter_context.get_op_name(relay_expr, ir_graph.QNN_OP_ELEMENT_WISE_ADD,
                                                ElementwiseBinaryOp.operation_to_legacy[ir_graph.QNN_OP_ELEMENT_WISE_BINARY_OPERATION_ADD])
...
#   SpaceToBatch
# ------------------------------------------------------------------------------
class RelaySpaceToBatchTranslation(RelayTranslationBase):
    def __init__(self):
        super(RelaySpaceToBatchTranslation, self).__init__()
    ...
    def extract_attributes(self,
        ...
        attr_dict["block_shape"] = relay_expr.attrs.block_shape
        attr_dict["pad_value"] = relay_expr.attrs.pad_value
        attr_dict["paddings"] = relay_expr.attrs.paddings

        log_debug3("\tblock_shape {}, pad_value {}, paddings {}",
                   attr_dict["block_shape"], attr_dict["pad_value"], attr_dict["paddings"])
    ...
    def translate_op(self,
        ...
        op_name = converter_context.get_op_name(relay_expr, SpaceToBatchOp.TRANSLATION_KEY,
                                                    SpaceToBatchOp.LEGACY_TRANSLATION_KEY)
        block_shape = attr_dict["block_shape"]
        if len(block_shape)!= 2:
            raise ValueError("Block Shape 2 is Supported. Unsupported Block Shape received: {}".format(block_shape))
        if isinstance(block_shape, tvm.runtime.ndarray.NDArray) or isinstance(block_shape, tvm.runtime.NDArray)\
                or isinstance(block_shape, tvm.ir.container.Array):
            block_shape = np.array(block_shape).astype('int')
            block_shape = [shape.item() for shape in block_shape]

        paddings = attr_dict["paddings"][:]

        for index, padding in enumerate(paddings):
            if isinstance(padding, tvm.ir.container.Array):
                padding = np.array(padding).astype('int')
                padding = [shape.item() for shape in padding]
            paddings[index]= padding

        ir_op = SpaceToBatchOp(op_name, block_shape=block_shape, paddings=paddings)
        return ir_op


RelayTranslations.register_translation(RelaySpaceToBatchTranslation(),
                                       converter_type('space_to_batch_nd', 'relay'))
