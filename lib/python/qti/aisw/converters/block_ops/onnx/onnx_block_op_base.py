...
class QnnOnnxBlockOp(metaclass=abc.ABCMeta):
    ...
    def __init__(self,
        ...
        """
        if (onnx_opset_version < self.min_opset
            or onnx_opset_version > self.max_opset):
            raise ValueError(f'BlockOp {self.name} only supports ONNX opset '
    ...
    def getOnnxScriptFunc(self) -> onnxscript.values.OnnxFunction:
        ...
        """
        raise ValueError(f'BlockOp {self.name} does not have an ONNX script '
