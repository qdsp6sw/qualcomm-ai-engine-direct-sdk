...
from qti.aisw.converters.block_ops.onnx.onnx_block_op_base\
    import QnnOnnxBlockOp, qcom_block_op_domain
from qti.aisw.converters.common.converter_ir.op_properties.masked_softmax\
    import (
...
class MaskedSoftmax(QnnOnnxBlockOp):
    ...
    def __init__(self,
        ...
        if aisw_opset_version != 1:
            raise ValueError(f'Operator {self.name} is only supported in '
    ...
    def getOnnxScriptFunc(self) -> OnnxFunction:
        ...
        if self.mode == MODE_UNCOMPRESSED_VAL:
            ...
            raise ValueError(f'{self.name} contains an unrecognized mode value '
