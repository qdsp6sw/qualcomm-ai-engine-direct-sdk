...
class QAIRTBinarySnooping(QAIRTSnooper):
    """Class that runs layer wise snooping."""

    def __init__(self, args, logger):
        super(QAIRTBinarySnooping, self).__init__("binary", args, logger)
        self.graph_result = {}
    ...
    def execute_graph(self, start_idx: int, end_idx: int, float_fallback: bool = True,
        ...
        '''
        custom_node_precision_path = self._args.quantization_overrides
        if create_subgraph:
        ...
        self.execute_on_qairt(
            ...
            calib_input_list=None, output_tensors=None, orig_model_outputs=self._args.output_tensor,
            ...
            float_fallback=float_fallback, quantization_overrides=custom_node_precision_path)

    def _load_fp32_results(self):
        reference_directory = self._args.golden_output_reference_directory
        # For 1 debugging input later will change to multiple inputs
        _fp32_outputs = {}
        for file_entry in os.scandir(reference_directory):
            file_entry_name = file_entry.name
            if file_entry_name.endswith('.raw'):
                _fp32_outputs[file_entry_name] = np.fromfile(file_entry.path, dtype=np.float32)
        self.fp32_outputs = _fp32_outputs
        ...
        for out_node in self._args.output_tensor:
            ...
            for quantized_output_directory in os.scandir(os.path.join(target_directory, 'output')):
                if os.path.isdir(quantized_output_directory):
                    target_tensor_path = os.path.join(quantized_output_directory.path,
                                                      activation_name)
                    reference_tensor = self.fp32_outputs[activation_name]
    ...
    def _is_search_candidate(self, tree1_score, tree2_score):
        ...
        if self._args.verifier.lower() == "mse":
            if tree1_score > self._args.subgraph_relative_weight * total_tree_score:
                return True
        elif self._args.verifier.lower() == "sqnr":
            if tree1_score < self._args.subgraph_relative_weight * total_tree_score:
                return True
    ...
    def run(self):
        ...
        if self._args.golden_output_reference_directory is None:
            self.trigger_framework_runner(add_layer_outputs=self._args.output_tensor)
