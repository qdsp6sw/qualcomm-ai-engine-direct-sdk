...
class GetTensorMappingRunnerWithFramework(FrameworkRunner):
    ...
    def generate_tensor_mapping(self,
                                output_dir):  # type: (Tuple[List[str], List[str]], str) -> None
        try:
            tensor_mapping = {}
            ...
            for _, _, file_list in outputs:
                for file_path in file_list:
                    ...
                    if self.engine == Engine.QNN.value:
                        sanitized_node_name = santize_node_name(
                            self.framework_instance.get_mapping_for_qnn_node(tensor_name))
                        # get_mapping_for_qnn_node() returns empty string if no mapping node is available from
                        # framework side. This condition ensures to filter out such empty entries into dict
                        if sanitized_node_name:
                            tensor_mapping[tensor_name] = santize_node_name(
                                self.framework_instance.get_mapping_for_qnn_node(tensor_name))
                    elif self.engine == Engine.SNPE.value:
                        mapping_name = self.framework_instance.get_mapping_for_snpe_node(
                            tensor_name)
                        if mapping_name:
                            tensor_mapping[tensor_name] = mapping_name
                    else:
                        raise Exception(
    ...
    def run(self, target_outputs_dir):
        self.load_framework_for_tensor_mapping()
        self.generate_tensor_mapping(target_outputs_dir)
...
class GetTensorMappingRunnerWithGolden():
    ...
    def get_mapping_for_snpe_node_with_golden_dir(self, snpe_output):
        ...
        for k in self.dict_of_golden_dir.keys():
            if k == snpe_output or k == "_" + snpe_output:
                return self.dict_of_golden_dir[k]
            ...
            for _, _, file_list in outputs:
                for file_path in file_list:
                    ...
                    if tensor_name in tensor_mapping:
                        continue
                    if self.engine == Engine.QNN.value:
                        mapping_name = self.get_mapping_for_qnn_node_with_golden_dir(tensor_name)
                        # mapping_name is empty string if no mapping node is available from
                        # framework side. This condition ensures to filter out such empty entries into dict
                        if mapping_name:
                            tensor_mapping[tensor_name] = mapping_name
                    elif self.engine == Engine.SNPE.value:
                        tensor_mapping[
                            tensor_name] = self.get_mapping_for_snpe_node_with_golden_dir(
                                tensor_name)
                    else:
                        raise Exception(
                            f"Tensor mapping is not supported for given engine/sdk type {self.engine}"
...
def TensorMapping(get_mapping_arg, logger):
    ...
    try:
        if get_mapping_arg.framework and get_mapping_arg.model_path:
            ...
            raise Exception(
                "Tensor_mapping with framework venv failed because model_path or framework is missing."
            )
    except Exception as e:
        # mapping tensor with golden_dir, if not, just output default json with no mapping.
        logger.warn("Try to generate tensor_mapping.json with framework env but failed: {}".format(
            str(e)))
