...
from qti.aisw.accuracy_debugger.lib.utils.nd_constants import Aarch64_windows_Architectures


class WosInterface(DeviceInterface):

    def __init__(self, logger=None, device_setup_path=None):
        """
        Initializes the Device class for Windows on Snapdragon(WoS).
        ...
        self._logger = logger
        self.device = Aarch64_windows_Architectures.wos.value
    ...
    def execute(self, commands, cwd='.', env=None):
        ...
        wos_shell_command = cd_cmd + f' if($?) {{ {dependency_check_cmd} }} ;'
        ...
        wos_shell_command += f' if($?) {{ {env_setup_cmd} }} ;'
        # In wos system, installation of executables change the present working directory
        # Hence changing the present working directory again
        cd_cmd_next = 'cd ' + cwd
        wos_shell_command += f' if($?) {{ {cd_cmd_next } }} ;'
        # Setup extra environment variables which are provided by user.
        for key, value in env.items():
            env[key] = value.replace('/', '\\')
            env_setup_command = '$env:{key} += "{path_sep}{value}"'.format(
                key=key, value=env[key], path_sep=os.pathsep)
            wos_shell_command += f' if($?) {{ {env_setup_command} }} ;'

        qnn_converter = False
        snpe_converter = False
        for cmd in commands:
            if 'converter' in cmd.split(' ')[0]:
                qnn_converter = True
            if 'to-dlc' in cmd.split(' ')[0]:
                snpe_converter = True

        # binaries_path Directory must have executables
        if qnn_converter:
            binaries_path = os.path.join(bin_path, 'arm64x-windows-msvc')
        elif snpe_converter:
            binaries_path = os.path.join(bin_path, 'x86_64-windows-msvc')
        else:
            binaries_path = os.path.join(bin_path, 'aarch64-windows-msvc')
        ...
        for cmd in commands:
            ...
            if '.exe' in cmd.split(' ')[0]:
                ...
                cmd = (sys.executable + ' ' +
                       '{binaries_path}\{cmd}'.format(binaries_path=binaries_path, cmd=cmd))
            wos_shell_command += f' if($?) {{ {cmd} }} ;'

        return nd_device_utilities.execute(wos_shell_command, shell=True, cwd=cwd, powershell=True)
